...
def test_VoxelgridCentersSampler_expected_values(
    ...
    sampler = VoxelgridCentersSampler(
        pyntcloud=simple_pyntcloud, voxelgrid_id=voxelgrid_id
    )
...
@pytest.mark.usefixtures("simple_pyntcloud")
def test_VoxelgridCentroidsSampler_expected_values(
    simple_pyntcloud, n_x, n_y, n_z, expected_n, expected_point
):
    voxelgrid_id = simple_pyntcloud.add_structure(
        "voxelgrid", n_x=n_x, n_y=n_y, n_z=n_z
...
@pytest.mark.usefixtures("simple_pyntcloud")
def test_VoxelgridNearestSampler_expected_values(
    ...
    sampler = VoxelgridNearestSampler(
        pyntcloud=simple_pyntcloud, voxelgrid_id=voxelgrid_id, n=n_points
    )
    sampler.extract_info()
...
def test_VoxelgridNearestSampler_keeps_original_scalar_fields(
    pyntcloud_with_rgb_and_normals,
):
    voxelgrid_id = pyntcloud_with_rgb_and_normals.add_structure("voxelgrid")
    sampler = VoxelgridNearestSampler(
        pyntcloud=pyntcloud_with_rgb_and_normals, voxelgrid_id=voxelgrid_id
    )
    sampler.extract_info()
...
def test_VoxelgridHighestSampler_keeps_original_scalar_fields(
    pyntcloud_with_rgb_and_normals,
):
    voxelgrid_id = pyntcloud_with_rgb_and_normals.add_structure("voxelgrid")
    sampler = VoxelgridHighestSampler(
