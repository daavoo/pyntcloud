...
def test_VoxelgridScalarField_raises_KeyError_if_id_is_not_valid(
    pyntcloud_with_voxelgrid_and_voxelgrid_id, voxelgrid_id
):
    cloud, true_id = pyntcloud_with_voxelgrid_and_voxelgrid_id
    scalar_field = VoxelgridScalarField(pyntcloud=cloud, voxelgrid_id=voxelgrid_id)
...
@pytest.mark.usefixtures("pyntcloud_with_voxelgrid_and_voxelgrid_id")
def test_VoxelgridScalarField_bounds(
    pyntcloud_with_voxelgrid_and_voxelgrid_id, scalar_field_class, min_val, max_val
):
    cloud, voxelgrid_id = pyntcloud_with_voxelgrid_and_voxelgrid_id
    scalar_field = scalar_field_class(pyntcloud=cloud, voxelgrid_id=voxelgrid_id)
    scalar_field.extract_info()
...
def test_EuclideanClusters_values(pyntcloud_with_clusters_and_voxelgrid_id):
    ...
    scalar_field = EuclideanClusters(pyntcloud=cloud, voxelgrid_id=voxelgrid_id)
    scalar_field.extract_info()
    with np.errstate(divide="ignore", invalid="ignore"):
        scalar_field.compute()
    scalar_field_values = next(iter(scalar_field.to_be_added.values()))
