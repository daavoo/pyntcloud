...
@pytest.mark.parametrize(
    ...
    [
        ("inclination_degrees", 0, 180),
        ("inclination_radians", 0, np.pi),
        ("orientation_degrees", 0, 360),
        ("orientation_radians", 0, np.pi * 2),
    ],
)
@pytest.mark.usefixtures("pyntcloud_with_rgb_and_normals")
def test_normal_scalar_fields_bounds(
    pyntcloud_with_rgb_and_normals, scalar_field_name, min_val, max_val
):
