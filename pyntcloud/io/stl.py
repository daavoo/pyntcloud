...
def read_stl(filename):
    ...
    #length2 = np.size(stl.normals.flatten(),0)
...
#    v2 = v2.reshape(length3,1)
    ...
    #stl_points = pd.DataFrame([1,1],0,columns=['x','y','z'])
    data = {}
#    data['points'] = pd.DataFrame(np.zeros([length1,6]),index=index1,columns=['x','y','z','nx','ny','nz'])
    data['points'] = pd.DataFrame(np.zeros([length1,3]),index=index1,columns=['x','y','z'])

    #normals = pd.DataFrame(np.zeros([length2,3]),index=index2,columns=['nx','ny','nz'])
...
#    data['mesh'] = pd.DataFrame(np.zeros([length3,3]),index=index3,columns=['v1','v2','v3'])
    ...
    data['points'].z = z
#    
#    data['points'].nx = np.NaN
#    data['points'].ny = np.NaN  
#    data['points'].nz = np.NaN
#    
#    pd.DataFrame.update(data['points'],{'nx':nx,'ny':ny,'nz':nz})
#        
#    data['mesh'].v1 = v0
#    data['mesh'].v2 = v1  
#    data['mesh'].v3 = v2
