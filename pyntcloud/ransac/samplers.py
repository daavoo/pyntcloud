...
class VoxelgridSampler(RansacSampler):
    ...
    def get_sample(self):
        ...
        """
        # pick one point and get it's voxel index 
        idx = random.randint(0, len(self.points))
        ...
        sample = np.random.choice(len(self.points), self.k, replace=False, p=points_in_voxel)
