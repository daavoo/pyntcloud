...
This page contains usefull information for developers that want to modify / add content to the scalar_fields module.
...
The sections bellow will guide you trough the scalar fields module explaining how you can create your own scalar fields or where you need
...
All filters must inherit from the base class `ScalarField` and implement it's abstract methods.
...
The `ScalarField.exctract_info` method must be overrided in order to extract and save in a attribute the information required to compute the scalar field.

See SUBMODULE BASE CLASS bellow for more information.

`ScalarField.compute` is where the new DataFrame columns are generated.

See SPECIFIC SCALAR FIELD CLASS bellow.
...
Scalar fields are grouped in submodules according to wich kind of information they require to be computed.
...
For example, in the sf_voxelgrid submodule there is a ScalarField_Voxelgrid class from wich all the scalar fields that require a VoxelGrid inherit.
