...
The sections bellow will guide you trough the samplers module explaining how you can create your own samplers or where you need
...
All samplers must inherit from the base class `Sampler` and implement it's abstract methods.
...
The `Sampler.exctract_info` method must be overrided in order to extract and save in a attribute the information required to generate the sample.

See SUBMODULE BASE CLASS bellow for more information.

`Sampler.compute` is where the sample is generated and returned. It should use the informaction extracted by the above method in order to generate the sample.

See SPECIFIC SAMPLER CLASS bellow.
...
Samplers are grouped in submodules according to wich kind of information they require to be computed.
...
For example, in the s_voxelgrid submodule there is a Sampler_Voxelgrid class from wich all the samplers that require a Voxelgrid inherit.
