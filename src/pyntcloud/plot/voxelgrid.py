...
def get_voxelgrid_pythreejs(xyz, colors):
    ...
    colors = pythreejs.InstancedBufferAttribute(array=colors, meshPerAttribute=3)
...
def plot_voxelgrid_with_matplotlib(voxelgrid, feature_vector, cmap="Oranges"):
    ...
    fig, axes = plt.subplots(
        int(np.ceil(z_dim / 4)), np.min((z_dim, 4)), figsize=(20, 20)
    )
    plt.tight_layout()
...
def plot_voxelgrid(
    voxelgrid, d=3, mode="binary", backend="pythreejs", cmap="Oranges", **kwargs
):
